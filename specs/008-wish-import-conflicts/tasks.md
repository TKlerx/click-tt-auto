...
- [x] T036 Filter the review to unresolved conflicts, added wishes, missing-from-import and accepted/no-op matches (FR-011). The spec originally named the four categories without defining two of them; both are now pinned down in FR-011 itself and in the 2026-07-16 clarification. "Added" is tracked by `RasterImportedWishRow.createdWish`, set on the row that brought the wish into existence.
- [x] T036a Separate a conflict that would undo a manual correction from one where the source value moved, and filter to each (FR-011a). `RasterWish.origin` already carries this; `partitionConflicts` in `webapp/src/lib/raster/wish-conflicts.ts` applies it. It lives in lib rather than the panel because there is no component-render harness here, and inverting the check would silently deprioritise exactly the wrong half.
