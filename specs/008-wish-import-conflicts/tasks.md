...
- [ ] T036 Filter the review to unresolved conflicts, added wishes, missing-from-import and accepted/no-op matches (FR-011). **Not done.** `WishImportReviewPanel` has no filter control, and `listWishImportReview` only queries unresolved conflicts (`decision: null`), unmatched rows and missing wishes — added wishes and accepted/no-op matches are never fetched, so two of the four required categories cannot be shown yet. Needs a decision on how each is defined before it can be built.
