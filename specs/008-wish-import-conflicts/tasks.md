...
- [x] T036 Filter the review to unresolved conflicts, added wishes, missing-from-import and accepted/no-op matches (FR-011). FR-011 named the four categories without defining them; two follow from the spec (`decision: null`, and FR-007a/b), the other two were settled by decision:
  - **added** = the wishes the *latest import batch* brought into existence, tracked by `RasterImportedWishRow.createdWish`. Scoped to the batch so it answers "what did this import do" and empties as imports settle. Rejected alternative: every `origin: IMPORTED` wish without a `reviewedAt` — on a fresh input set that is the entire roster, and nothing in the panel can mark one reviewed, so the count would never fall.
  - **accepted/no-op** = one "settled" filter, since neither side wants a decision: conflicts carrying a decision, plus latest-batch rows that paired with a wish and raised no conflict.
