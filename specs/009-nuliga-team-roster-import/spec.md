...
- **FR-015a**: The CLI MUST verify the downloaded export is the Meisterschaft it asked for — its `Region` and `Saison` matching the selection — **before saving it**, and MUST discard and report a mismatch. This is not paranoia: `AGENTS.md` records that nuLiga admin URLs "contain stateful click counters and can return the wrong group/PDF when opened later or out of sequence", and that a downloaded PDF must be checked against the clicked page title before being trusted. The same hazard applies to this export, and the same practice answers it. FR-005 catches a mismatch at import, but that is the webapp finding a mistake the CLI made — far from where it happened, and after it has been carried around.
- **FR-015b**: The CLI MUST reach the export by navigating the live admin UI, not by replaying a collected URL, for the reason `AGENTS.md` gives.
