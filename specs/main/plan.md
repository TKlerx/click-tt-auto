...
Build a Playwright-based CLI tool in TypeScript that automates the approval of table tennis match reports in the click-TT admin webapp (WTTV). The tool logs in, navigates to the match list, filters for unapproved matches, iterates through all pages, validates each match against 4 rules (status, error messages, MF presence, 6 players), approves valid matches, and produces a detailed report of all actions taken.

## Technical Context

**Language/Version**: TypeScript 5.x, Node.js LTS (22.x)
**Primary Dependencies**: Playwright (browser automation), dotenv (credentials), minimist (CLI args)
**Storage**: JSON report files written to `reports/` directory
...
| V. Observable Output | PASS | Stdout summary + JSON report file |
...
**Structure Decision**: Single flat `src/` directory — the tool is simple enough that nested modules are unnecessary. Each file has a clear single responsibility. Validation logic is isolated in `validator.ts` for easy unit testing without browser dependencies.
