...
- [X] T007 [P] [US1] Integration test `webapp/tests/integration/raster-source-workspace.test.ts` covering S1 (source saved with `inputSetId`), S2 (add disabled with no workspace), S6 (list filtered by workspace), S5 (`SCOPE_USER` source write → 403).
- [X] T008 [US1] Add-source panel (click-TT URL + wish PDF) defaulting to current scope+season+selected workspace, with no scope picker and the season visible before submit (FR-001, FR-002, FR-003, FR-003a, FR-004, FR-009a). Depends: T002, T005.
- [X] T009 [P] [US1] i18n keys for the add-source panel in `webapp/src/i18n/messages/{en,de,es,fr,pt}.json`.
...
- [X] T011 [US2] Workspace selector + prominent create-first action near the page context, wired to the T005 resolution; source actions unavailable until a workspace is selected (FR-005, FR-006, FR-006a, FR-007, FR-007a, FR-008, FR-008a, FR-009). Depends: T003, T005.
- [X] T012 [P] [US2] i18n keys for the workspace selector / create action in `webapp/src/i18n/messages/{en,de,es,fr,pt}.json`.
...
- [X] T016 [P] [US4] Test `webapp/tests/integration/raster-workspace-switch.test.ts`: create a second workspace, switch, and confirm the source list follows the active workspace (FR-010, FR-017, US4 scenarios).
- [X] T017 [US4] Create-additional-workspace action + switching that updates source/validation/review context, keeping the active workspace visible to prevent wrong-workspace edits (FR-010, FR-017). Depends: T011.

## Phase 7: Polish & Cross-Cutting

- [X] T018 [P] Responsive layout + toast feedback pass on the import view; verify on a narrow viewport (constitution VIII, X).
