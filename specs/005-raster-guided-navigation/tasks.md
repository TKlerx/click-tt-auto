...
- [ ] T005 Generate the drop-and-recreate migration in `webapp/prisma/migrations-postgres/` (no data conversion, per FR-024) and regenerate the Prisma client.
...
- [ ] T047 [P] Verify no `district` string survives in `webapp/src/` outside migration history, and no user-facing "District" label survives in `webapp/src/i18n/messages/`.
