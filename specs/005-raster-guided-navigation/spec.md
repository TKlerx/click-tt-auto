...
- **Q4 (resolved 2026-07-14 at planning — declined)**: Should a snapshot produced from a run with groups excluded be marked as partial? Not in this feature. Marking a snapshot changes snapshot contents, which this spec's Out of Scope forbids, so an affirmative answer would need a spec amendment rather than a planning decision. The hazard is real and is stated as residual risk in `research.md` R-008: a scheduler opening a partial run's snapshot from run comparison has nothing telling them it is partial. It is the same hazard feature 006 must solve for combined snapshots (its FR-020/FR-021), and it should be solved once across both rather than bolted on here. Within the guided flow, FR-006e, FR-006g, FR-011a and SC-013 already prevent the confusion.
