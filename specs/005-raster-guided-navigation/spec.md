...
- **Q4 (resolved 2026-07-14 at planning — declined here; owned by feature 006 as of 2026-07-15)**: Should a snapshot produced from a run with groups excluded be marked as partial? Not in this feature. Marking a snapshot changes snapshot contents, which this spec's Out of Scope forbids, so an affirmative answer would need a spec amendment rather than a planning decision. Within the guided flow, FR-006e, FR-006g, FR-011a and SC-013 already prevent the confusion; the gap was strictly outside it, in run comparison and snapshot lists. **Feature 006 now specifies the answer**: its FR-030 to FR-038 define a per-run coverage record, and FR-035 applies it to single-scope runs explicitly — a Bezirk run with excluded groups is marked incomplete by the same rule as a combined subset run. This feature still builds none of it; see `research.md` R-008.
