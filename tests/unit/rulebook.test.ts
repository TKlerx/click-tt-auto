...
import { derbySpieltag, relation } from "../../src/raster/rulebook/index.js";
...
describe("raster rulebook", () => {
  it("matches the verified 12er reference homes", () => {
    expect(deriveHomeWeeks(10, 1).weeks).toEqual([1, 2, 4, 6, 8, 13, 15, 17, 19]);
    expect(deriveHomeWeeks(12, 1).weeks).toEqual([1, 2, 4, 6, 8, 10, 13, 15, 17, 19, 21]);
    expect(deriveHomeWeeks(12, 6).weeks).toEqual([1, 3, 5, 7, 9, 11, 13, 14, 18, 20]);
    expect(deriveHomeWeeks(12, 12).weeks).toEqual([2, 4, 6, 8, 10, 11, 13, 15, 19, 20]);
    expect(deriveHomeWeeks(14, 1).weeks).toEqual([1, 2, 4, 6, 8, 10, 12, 15, 17, 19, 21, 23, 25]);
  });
