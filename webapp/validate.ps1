    ...
    try {
        ...
        switch -Regex ($message) {
            ...
            '^packageManager does not pin pnpm$' {
                Write-Host "Set package.json `packageManager` to the pinned pnpm version, for example `pnpm@11.8.0`." -ForegroundColor Yellow
            }
            '^pnpm is not available$' {
                Write-Host "Install pnpm through Corepack (`corepack enable` then `corepack prepare pnpm@11.8.0 --activate`)." -ForegroundColor Yellow
            }
